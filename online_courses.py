class Student:
    ...
    def __str__(self):
        ...
        self.average_rating = sum(map(sum, self.grades.values())) \
            / grades_count
...
best_lecturer_2 = Lecturer('Анатолий', 'Корсаков')
best_lecturer_2.courses_attached += ['Java']
best_lecturer_3 = Lecturer('Евгений', 'Шмаргунов')
best_lecturer_3.courses_attached += ['Python']

cool_reviewer_1 = Reviewer('Александр', 'Бардин')
cool_reviewer_1.courses_attached += ['Python']
cool_reviewer_1.courses_attached += ['Java']
cool_reviewer_2 = Reviewer('Алена', 'Хохлова')
cool_reviewer_2.courses_attached += ['Python']
cool_reviewer_2.courses_attached += ['Java']

student_1 = Student('Нина', 'Свиридова')
student_1.courses_in_progress += ['Python']
student_1.finished_courses += ['Введение в программирование']
student_2 = Student('Владимир', 'Попов')
student_2.courses_in_progress += ['Java']
student_2.finished_courses += ['Введение в программирование']
student_3 = Student('Петр', 'Киринов')
...
print('Перечень студентов: ')
print('-' * len('Перечень студентов:'))
print(f'{student_1}\n\n{student_2}\n\n{student_3}')
print()
print()

print('Перечень лекторов: ')
print('-' * len('Перечень лекторов:'))
print(f'{best_lecturer_1}\
    \n\n{best_lecturer_2}\n\n{best_lecturer_3}')
print()
print()

print('Перечень проверяющих:')
print('-' * len('Перечень проверяющих:'))
print(f'{cool_reviewer_1}\n\n{cool_reviewer_2}')
print()
print()

print('Результат сравнения студентов (по средним оценкам за ДЗ):')
print('-' * len('Результат сравнения студентов (по средним оценкам за ДЗ):'))

s1 = student_1
s2 = student_2
s3 = student_3

print('\n'.join(f'{i}' for i in sorted([s1, s2, s3], reverse=True)))

print()
print()

print('Результат сравнения лекторов (по средним оценкам за лекции):')
print('-' * len('Результат сравнения лекторов (по средним оценкам за лекции):'))

l1 = best_lecturer_1
l2 = best_lecturer_2
l3 = best_lecturer_3

print('\n'.join(f'{j}' for j in sorted([l1, l2, l3], reverse=True)))
print()
print()
...
print(f"Средняя оценка для всех лекторов по курсу \
{'Python'}: {lecturer_rating(lecturer_list, 'Python')}")
print()
print()
print('*' * len('Для выхода нажмите ENTER'))
